...
1. **Classification drives the path**: terminal errors skip retries
   ...
   retryable errors go around ([05 §2](../05-errors/02-error-design.md)'s
   classification, [18 §3](../18-kafka-with-go/03-producer-consumer.md)'s
   `Terminal()` marker).
...
from [15 §4](../15-microservices/04-idempotency-sagas-outbox.md) and
[18 §3](../18-kafka-with-go/03-producer-consumer.md) is the shared
...
- **The key namespace travels in the message** (header/attribute):
  ...
  ([25 §3](../25-fintech-with-go/03-integrity-and-exactly-once.md)'s
  threading rule).
- **Missing key = terminal**: an unkeyed message cannot be made
  idempotent after the fact; reject it at the door ([18 §3](../18-kafka-with-go/03-producer-consumer.md)'s
  `ErrMalformed` decision, worth restating because every team rediscovers it).
- **The claim store is the storage tier** ([16 §5](../16-distributed-systems/05-delivery-backpressure-shedding.md)'s
...
([16 §4](../16-distributed-systems/04-quorums-sharding.md)'s shard-key
rules). The broker mappings:

| Broker | The per-key mechanism |
|---|---|
| Kafka | key → partition; one consumer per partition ([18 §1](../18-kafka-with-go/01-kafka-concepts.md)) |
...
- **Replay from DLQ**: triage the metadata (error class, attempt
  ...
  ([18 §3](../18-kafka-with-go/03-producer-consumer.md)'s replay
  discipline).
...
- **Retrying terminal failures**: every malformed message retried
  ...
  cheap fix ([05 §2](../05-errors/02-error-design.md)'s table).
...
- The `Handler`/`Terminal()` contract shared with
  [18 §3](../18-kafka-with-go/03-producer-consumer.md): one
...
- Backoff during redelivery consumes queue capacity: a poison
  ...
  it ([16 §5](../16-distributed-systems/05-delivery-backpressure-shedding.md)'s
  shedding logic in miniature).
- Batch fetch/ack where the broker offers it ([18 §4](../18-kafka-with-go/04-observability-tuning.md)'s
...
- The redelivery race: two consumers, one message's original and
  ...
  real job ([08 §4](../08-concurrency/04-sync-primitives.md)'s
  single-winner principle at the storage layer).
- Graceful drain: stop fetching, finish in-flight handlers, ack,
  then exit ([12 §5](../12-http-networking/05-graceful-shutdown.md)'s
...
- Fault tiers: handler panics (recovered, dead-lettered), storage
  down (retryable storm, then breaker [15 §3](../15-microservices/03-resilience-patterns.md)),
