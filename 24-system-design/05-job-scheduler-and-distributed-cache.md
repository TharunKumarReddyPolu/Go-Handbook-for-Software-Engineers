...
when ownership is ambiguous ([16 §3](../16-distributed-systems/03-leader-election-leases-fencing.md)'s
leases, [16 §4](../16-distributed-systems/04-quorums-sharding.md)'s
...
**Scheduler**: jobs table + runs table ([16 §3](../16-distributed-systems/03-leader-election-leases-fencing.md)'s
lease claim in SQL):
...
§3](../16-distributed-systems/03-leader-election-leases-fencing.md)'s
tested coordinator) stops the zombie scheduler from double-running.

**Cache**: consistent hashing ring over nodes ([16 §4](../16-distributed-systems/04-quorums-sharding.md)):
...
- **Cache**: capacity scales by adding nodes (ring absorbs
  ...
  ([19 §3](../19-performance/03-concurrency-performance.md)'s
  tested pattern) and jittered TTLs prevent synchronized expiry
  ([13 §5](../13-databases/05-caching-with-redis.md)).
- **Invalidation** is the cache's hard problem: write-through for
  correctness-critical keys, delete-on-write ([13 §5](../13-databases/05-caching-with-redis.md)'s
...
|---|---|---|
| Dispatcher crashes mid-batch | Lease expires; another claims; zombie blocked by fencing | [16 §3](../16-distributed-systems/03-leader-election-leases-fencing.md)'s coordinator |
| Job crashes the worker | Attempt count + DLQ; poison jobs quarantined | [17 §3](../17-messaging/03-portable-patterns.md)'s retry/DLQ shape |
...
users notice ([20 §2](../20-observability/02-metrics.md)). Cache:
hit ratio per node, singleflight wait time, eviction rate.
Security: job payloads are data, not code (no eval; handlers are a
registered, allowlisted set: [21 §1](../21-security/01-threat-model-and-validation.md)'s
...
- **The stage-5 job processor ([08
  §5](../08-concurrency/05-patterns.md))** is the worker loop;
...
- **`SKIP LOCKED` via `database/sql`** needs a transaction per
  claim ([13 §2](../13-databases/02-transactions-and-isolation.md)'s
...
- **The ring**: `hash/crc32` + sorted-node ring is a teaching
  ...
  §3](../19-performance/03-concurrency-performance.md) is
  unchanged either way.
- **Clock discipline**: leases compare against the *database's*
  clock where possible (the SQL above) to avoid fleet clock skew
  becoming double-execution ([16 §3](../16-distributed-systems/03-leader-election-leases-fencing.md)'s
