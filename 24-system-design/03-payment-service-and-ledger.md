...
reconciliation) are built and tested in [25 Section 1-3](../25-fintech-with-go/01-money-and-payments.md);
...
|---|---|
| Charge a payment method exactly-once | idempotency keys ([25 Section 3](../25-fintech-with-go/03-integrity-and-exactly-once.md)) |
| Balances never drift | double-entry ledger, invariant in SQL ([25 Section 2](../25-fintech-with-go/02-double-entry-ledger.md)) |
...
The idempotency contract ([25 Section 3](../25-fintech-with-go/03-integrity-and-exactly-once.md)):
...
transaction ([25 Section 2](../25-fintech-with-go/02-double-entry-ledger.md)'s
tested pattern; [13 Section 2](../13-databases/02-transactions-and-isolation.md)'s
`WithinTx`).
...
The ordering rules that make it correct ([25 Section 3](../25-fintech-with-go/03-integrity-and-exactly-once.md)'s
tested relay):
...
4. Events flow from the outbox, never from the handler: downstream
   ...
   idempotency ([18 Section 1](../18-kafka-with-go/01-kafka-concepts.md)).
...
- Balances are derived (SUM over entries) and cached with
  invalidation on write ([13 Section 5](../13-databases/05-caching-with-redis.md)):
...
| Duplicate event delivery | Consumers idempotent by event ID | dedupe ([18 Section 3](../18-kafka-with-go/03-producer-consumer.md)) |
| PSP timeout (unknown outcome!) | Reconcile asks the PSP by idempotency key | the unknown-response discipline ([16 Section 1](../16-distributed-systems/01-failure-model-cap-pacelc.md)) |
| Ledger drift (bug) | Nightly reconcile pages before money is wrong | reconciliation is the SLO ([25 Section 3](../25-fintech-with-go/03-integrity-and-exactly-once.md)) |

## Observability & security

Business metrics ([20 Section 2](../20-observability/02-metrics.md)):
`charges_total{status}`, `psp_latency`, `reconcile_diff_count`
(the metric that must always be zero). Spans per charge with
business attributes ([20 Section 3](../20-observability/03-tracing-and-otel.md)).
Security: amounts are integer minor units ([25 Section 1](../25-fintech-with-go/01-money-and-payments.md)),
ownership checked in the service ([21 Section 2](../21-security/02-authentication-and-authorization.md)),
audit logs append-only ([25 Section 4](../25-fintech-with-go/04-risk-and-compliance.md)'s
regulatory honesty).

## Go implementation considerations

- **The ledger example from [25 Section 2](../25-fintech-with-go/02-double-entry-ledger.md)**
...
- **Status machines as types**: charge status is a Go enum with an
  ...
  awkward and runtime-tested ([05 Section 2](../05-errors/02-error-design.md)'s
  domain errors per transition).
- **Latency budget**: idem claim + tx + PSP is the 300ms; the PSP
  call gets the remainder as its timeout ([15 Section 3](../15-microservices/03-resilience-patterns.md)'s
  budget hierarchy propagated per hop).
- **Testing shape**: the [25 Section 2](../25-fintech-with-go/02-double-entry-ledger.md)
