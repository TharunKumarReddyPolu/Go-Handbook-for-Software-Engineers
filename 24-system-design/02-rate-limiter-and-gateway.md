...
Section 4](../21-security/04-limits-and-hardening.md)) and resilience
([15 Section 3](../15-microservices/03-resilience-patterns.md)) chapters
...
GET  /livez /readyz            (edge's own health [22 Section 2])
...
operation ([13 Section 2](../13-databases/02-transactions-and-isolation.md)'s
...
Section 5](../16-distributed-systems/05-delivery-backpressure-shedding.md))
...
- Redis down: fail open or closed? **Closed for anonymous
  ...
  Section 4](../22-production-go/04-dependency-failures.md)'s class
...
| Backend brownout | gateway breakers open; 503 fast | breaker per backend ([15 Section 3](../15-microservices/03-resilience-patterns.md)) |
| Thundering retry after outage | jittered Retry-After; client backoff ([15 Section 3](../15-microservices/03-resilience-patterns.md)) | no synchronized wakeups |

## Observability & security

429s are traffic ([20 Section 2](../20-observability/02-metrics.md)):
...
Section 3](../21-security/03-tls-certificates.md)), verifies tokens once
([21 Section 2](../21-security/02-authentication-and-authorization.md)),
...
- **Identity propagation**: verify at the edge, forward claims as
  a signed internal header or mTLS identity ([21 Section 2](../21-security/02-authentication-and-authorization.md));
...
- **Timeout budgets at the edge**: the gateway owns the client's
  ...
  Section 3](../15-microservices/03-resilience-patterns.md)'s budget
  hierarchy; [12 Section 4](../12-http-networking/04-clients-and-timeouts.md)'s
  client construction).
- **Shedding is a Go win**: bounded queues + `select`-with-default
  make "reject now" a one-liner ([08 Section 5](../08-concurrency/05-patterns.md));
  the storm arithmetic ([15 Section 3](../15-microservices/03-resilience-patterns.md))
  is the test.
- **This repo's pattern map**: [21 Section 4](../21-security/04-limits-and-hardening.md)'s
  `Limiter` + `Fetcher`, [15 Section 3](../15-microservices/03-resilience-patterns.md)'s
