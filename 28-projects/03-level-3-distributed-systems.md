...
- **Infrastructure is allowed now, with stated reasons.** Kafka via
  franz-go ([18 §2](../18-kafka-with-go/02-go-clients.md)), Postgres,
...
- **Delivery semantics are explicit.** Each project states at-least-
  ...
  pin it ([18 §1](../18-kafka-with-go/01-kafka-concepts.md),
...
- Every consumer is idempotent: a test delivers the same message
  twice and asserts one effect ([15 §4](../15-microservices/04-idempotency-sagas-outbox.md)).
...
| Milestone | Done when |
|---|---|
| M1: order core | API + Postgres with the order state machine enforced in the domain layer ([25 §1](../25-fintech-with-go/01-money-and-payments.md)'s transition discipline) |
| M2: outbox | state change + outbox row in one transaction; a relay publishes and marks sent ([15 §4](../15-microservices/04-idempotency-sagas-outbox.md)) |
...
interviewers sit up (see [26 §4](../26-go-interview-preparation/04-senior-scenarios.md)).
...
| Milestone | Done when |
|---|---|
| M1: producer | acks=all, idempotence on, compression chosen with a reason ([18 §3](../18-kafka-with-go/03-producer-consumer.md)) |
| M2: consumer group | partition-assigned processing, offsets committed after side effects, rebalance survival |
| M3: failure routing | retriable failures retried with backoff; poison messages dead-lettered with triage metadata ([17 §3](../17-messaging/03-portable-patterns.md)) |
...
becomes your test suite ([25 §3](../25-fintech-with-go/03-integrity-and-exactly-once.md)).
...
| Milestone | Done when |
|---|---|
| M1: lease claiming | `SKIP LOCKED` claim with lease expiry ([16 §3](../16-distributed-systems/03-leader-election-leases-fencing.md)) |
| M2: fencing | a worker holding an expired lease cannot corrupt a successor: fencing tokens enforced at the write path |
| M3: scheduling | cron-like periodic jobs and one-shot jobs share the queue; jitter prevents thundering herds ([24 §5](../24-system-design/05-job-scheduler-and-distributed-cache.md)) |
...
**The lesson:** this project is [16 §3]'s lease theory compiled to
...
| Milestone | Done when |
|---|---|
| M1: lanes | critical/transactional/marketing lanes with different delivery guarantees and shed order ([24 §4](../24-system-design/04-notification-system.md)) |
| M2: dedup | idempotency keys at intake *and* channel level; a test proves a double-submit is one email |
| M3: vendor discipline | per-vendor rate limiters, retry-with-backoff on 429s, webhook receiver with signature verification ([21 §1](../21-security/01-threat-model-and-validation.md)) |
...
lesson is [24 §4]; the operational lesson is that your most important
...
| M2: windows | tumbling-window aggregation from keyed streams ([24 §6](../24-system-design/06-event-processing-and-analytics.md)) |
| M3: query + alert | windowed queries and simple threshold alerts; the SLO vocabulary from [20 §4](../20-observability/04-slos-and-alerting.md) in the alert definitions |
| M4: self-observability | the platform exposes its own RED metrics ([20 §1](../20-observability/01-structured-logging.md)); ingestion benchmarks find the ceiling ([19 §1](../19-performance/01-measure-first.md)) |
...
- [17 §4](../17-messaging/04-schemas-evolution-and-testing.md): schema evolution discipline every M2+ consumer here needs
