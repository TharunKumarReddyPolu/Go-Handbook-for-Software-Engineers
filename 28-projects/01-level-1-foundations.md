...
- **Standard library only.** No web frameworks, no config libraries,
  ...
  [06 §4](../06-packages-modules/04-dependency-hygiene.md)).
...
- Errors are values with a taxonomy: sentinel or typed, wrapped with
  `%w`, classified retryable vs not ([05 §1](../05-errors/01-errors-are-values.md),
  [05 §2](../05-errors/02-error-design.md)).
- No goroutine you start can outlive its owner: every spawn has a
  stop ([08 §1](../08-concurrency/01-goroutines-and-channels.md),
  [09 §5](../09-memory-runtime/05-memory-leaks.md)).
...
| Milestone | Done when |
|---|---|
| M1: skeleton | `flag` parsing, `--help` text, exit codes: 0 ok, 1 usage, 2 runtime ([01 §2](../01-go-fundamentals/02-toolchain-and-workflow.md)) |
| M2: core logic | pure functions handle the transformation; unit-tested as a package, independent of `main` |
| M3: streaming | files processed with `bufio.Scanner`, constant memory on a 1 GB input ([02 §1](../02-go-language/01-arrays-and-slices.md): no read-all) |
| M4: polish | structured errors to stderr, `defer` hygiene ([01 §7](../01-go-fundamentals/07-defer-panic-recover.md)), fuzz the parser for panics ([10 §4](../10-testing/04-benchmarks-coverage-fuzzing.md)) |
...
[24 §1](../24-system-design/01-url-shortener.md); here you build the
simplest honest version of it.

| Milestone | Done when |
|---|---|
| M1: routing | `net/http` ServeMux patterns, health endpoint ([12 §1](../12-http-networking/01-handlers-and-routing.md)) |
| M2: store | an interface `Store` with an in-memory impl; collision handling with retry |
| M3: JSON boundary | create returns 201 + location; validation errors are 4xx with a stable body shape ([12 §3](../12-http-networking/03-json-and-rest-apis.md), [05 §2](../05-errors/02-error-design.md)) |
| M4: redirects | 302 until you decide permanence, open-redirector trap documented ([24 §1](../24-system-design/01-url-shortener.md)) |
| M5: tests | `httptest` end-to-end tests of the API surface ([10 §2](../10-testing/02-doubles-and-httptest.md)) |

**The lesson:** the store interface is dependency inversion in
miniature ([04 §5](../04-functions-methods-interfaces/05-dependency-inversion.md)):
...
| Milestone | Done when |
|---|---|
| M1: domain first | types and invariants before HTTP exists ([04 §6](../04-functions-methods-interfaces/06-constructors-and-invariants.md)); zero values chosen deliberately ([01 §5](../01-go-fundamentals/05-zero-values.md)) |
| M2: service layer | business rules in a service, transport as a thin translator |
| M3: error mapping | domain errors → status codes in one table; 401/403/404 discipline if you add identity ([21 §2](../21-security/02-authentication-and-authorization.md)) |
| M4: middleware | request ID + logging + panic recovery as composable wrappers ([12 §2](../12-http-networking/02-middleware.md)) |
| M5: full suite | table tests per endpoint, concurrent-access test with `-race` |

**The lesson:** the layering from [14 §1](../14-backend-development/01-service-layout.md)
...
| Milestone | Done when |
|---|---|
| M1: streaming core | line/chunk pipeline with backpressure: the producer blocks, memory stays flat ([08 §2](../08-concurrency/02-select-and-timeouts.md)) |
| M2: fan-out pool | stage-2/3 worker pool with a results channel and ordered or unordered output, your choice documented ([08 §5](../08-concurrency/05-patterns.md)) |
| M3: cancellation | context plumbed end to end; Ctrl-C drains cleanly ([08 §3](../08-concurrency/03-context.md)) |
| M4: failure semantics | per-item errors collected, not fatal; poison items dead-lettered to a reject file |
| M5: proof | `-race` clean; a benchmark before/after pool sizing ([10 §4](../10-testing/04-benchmarks-coverage-fuzzing.md)) |
...
pool version prevents ([08 §7](../08-concurrency/07-pitfalls.md)).
