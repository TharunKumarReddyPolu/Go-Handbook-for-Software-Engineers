...
- Degrade honestly: every project must answer "what do you do when
  ...
  ([22 Section 4](../22-production-go/04-dependency-failures.md)).
...
- Every concurrency bound has a benchmark or test that proves it holds
  under contention ([19 Section 3](../19-performance/03-concurrency-performance.md)).
- Every timeout has a value, a source (config), and a test that fires
  it ([08 Section 2](../08-concurrency/02-select-and-timeouts.md)).
...
| Milestone | Done when |
|---|---|
| M1: queue core | bounded channel + worker pool with graceful drain: stage-5 pattern as a service ([08 Section 5](../08-concurrency/05-patterns.md)) |
| M2: retries | per-job attempt count, exponential backoff with jitter, context-aware cancellation mid-retry |
| M3: DLQ shape | exhausted jobs land in a inspectable dead-letter list with triage metadata (the shape from [17 Section 3](../17-messaging/03-portable-patterns.md)) |
...
**The lesson:** this project is [17 Section 1](../17-messaging/01-queues-logs-pubsub.md)'s
...
| Milestone | Done when |
|---|---|
| M1: fetch loop | `http.Client` with real timeouts and transport tuning ([12 Section 4](../12-http-networking/04-clients-and-timeouts.md)) |
| M2: politeness | per-host concurrency = 1..2, global semaphore, robots awareness as a stated policy |
| M3: dedup + frontier | visited set ([03 Section 2](../03-data-structures/02-sets.md)) and a bounded frontier; behavior when the frontier hits its cap is documented |
| M4: shutdown | SIGINT drains in-flight fetches, writes partial results, exits nonzero on error budget breach |
| M5: proof | `-race` clean at 500 URLs; goroutine count flat over time (leak check per [09 Section 5](../09-memory-runtime/05-memory-leaks.md)) |
...
| Milestone | Done when |
|---|---|
| M1: two-layer limiter | global + identity-keyed token buckets; the exact design and its tests exist in [21 Section 4](../21-security/04-limits-and-hardening.md): build yours to the same contract |
| M2: HTTP enforcement | middleware that answers 429 with `Retry-After`, and load-sheds before it collapses ([16 Section 5](../16-distributed-systems/05-delivery-backpressure-shedding.md)) |
| M3: degraded mode | behavior when backing store is down: fail-closed for anonymous, documented policy for authenticated ([21 Section 4](../21-security/04-limits-and-hardening.md)) |
...
encoded in code. The design walkthrough is [24 Section 2](../24-system-design/02-rate-limiter-and-gateway.md);
...
| Milestone | Done when |
|---|---|
| M1: cache-aside | read path caches with TTLs; write path invalidates ([13 Section 5](../13-databases/05-caching-with-redis.md)) |
| M2: singleflight | concurrent misses collapse into one backend fetch ([19 Section 3](../19-performance/03-concurrency-performance.md)) |
...
| Milestone | Done when |
|---|---|
| M1: password storage | PBKDF2/argon2 hashing with per-user salt and versioned parameters ([21 Section 5](../21-security/05-secrets-and-supply-chain.md)) |
| M2: sessions vs tokens | pick one model, justify in the README using [21 Section 2](../21-security/02-authentication-and-authorization.md)'s table; implement issuance, expiry, revocation |
| M3: transport discipline | constant-time comparisons, no tokens in URLs or logs, authz gate separate from authn ([14 Section 4](../14-backend-development/04-authn-authz-and-validation.md)) |
...
first ([21 Section 1](../21-security/01-threat-model-and-validation.md)),
then build.
...
- [19 Section 3](../19-performance/03-concurrency-performance.md): the contention benchmarks that back your limit claims
