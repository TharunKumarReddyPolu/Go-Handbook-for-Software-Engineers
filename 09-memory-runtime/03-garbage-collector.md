...
guidance lives in [19 Section 2](../19-performance/02-memory-and-allocations.md)
(set GOMEMLIMIT, reduce allocations) and [22
Section 3](../22-production-go/03-resource-limits.md) (the container
...
- **Write barriers**: while marking runs *concurrently* with your
  ...
  correct while mutators run ([23 Section 6](../23-go-internals/06-memory-model.md)'s
...
two knobs compose ([19 Section 2](../19-performance/02-memory-and-allocations.md)):
...
Section 2](../19-performance/02-memory-and-allocations.md)'s before/after
numbers).
...
Section 3](../22-production-go/03-resource-limits.md)'s throttling
confusion is the sibling failure). Measure with
`/gc/pauses:seconds` ([20 Section 2](../20-observability/02-metrics.md))
...
capacity reviews ([20 Section 2](../20-observability/02-metrics.md)'s
...
[22 Section 3](../22-production-go/03-resource-limits.md)'s OOM story).
...
the box has room ([22 Section 3](../22-production-go/03-resource-limits.md)'s
...
Section 3](../22-production-go/03-resource-limits.md)'s knob table in
...
Section 4](../20-observability/04-slos-and-alerting.md)'s slow-burn
tier). Go 1.25+'s container-aware default (~90% automatic, [23
Section 3](../23-go-internals/03-runtime-architecture.md)) makes the
...
| Ignoring pointer density | Marking scans pointers; pointer-rich structures cost more | Slice structs, not slices of pointers, in bulk ([19 Section 2](../19-performance/02-memory-and-allocations.md)) |
...
- Allocation reduction is the GC tuning ([19
  Section 2](../19-performance/02-memory-and-allocations.md)'s playbook);
...
- Keep the live set small: caches bounded ([13
  Section 5](../13-databases/05-caching-with-redis.md)), sessions with
...
and design change ([19 Section 2](../19-performance/02-memory-and-allocations.md)'s
benchmarks; [08 Section 4](../08-concurrency/04-sync-primitives.md)'s
pool semantics).
...
Section 5](../23-go-internals/05-interfaces-slices-strings.md)'s string
notes; [14 Section 2](../14-backend-development/02-configuration-and-secrets.md)'s
...
containers ([22 Section 5](../22-production-go/05-deploying-kubernetes.md)'s
hardening rows).
...
- Regression benchmarks with `-benchmem` ([10
  Section 4](../10-testing/04-benchmarks-coverage-fuzzing.md)): allocation
  count is the GC's input; pin it.
- Chaos: cap memory hard in staging ([22
  Section 3](../22-production-go/03-resource-limits.md)'s 64Mi exercise)
