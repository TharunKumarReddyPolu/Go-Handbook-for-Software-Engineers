...
[02 Section 2](../02-go-language/02-maps.md) and [08
Section 1](../08-concurrency/01-goroutines-and-channels.md); this is the
...
Section 2](../08-concurrency/02-select-and-timeouts.md) is this scan
...
- **Randomized iteration**: the runtime picks a random bucket and
  ...
  Section 2](../02-go-language/02-maps.md)'s discipline follows).
...
forever ([08 Section 7](../08-concurrency/07-pitfalls.md)): `chansend` on
...
smoothing*, not speed ([08 Section 5](../08-concurrency/05-patterns.md)'s
backpressure math).
...
Section 4](../08-concurrency/04-sync-primitives.md)'s table) or a mutex
...
- **Close is a broadcast**: `closechan` wakes *every* waiter on
  ...
  ([08 Section 5](../08-concurrency/05-patterns.md)'s cancellation
  broadcast).
...
| `len(ch)` as a backpressure signal | qcount is a stale snapshot | Token buckets / semaphores ([21 Section 4](../21-security/04-limits-and-hardening.md)) |
...
- Never share a channel's ownership: sender closes, receiver
  ranges ([08 Section 1](../08-concurrency/01-goroutines-and-channels.md)).

## Performance Considerations

- Channel ops take one mutex on the whole `hchan`: hot single-
  channel fan-in can contend ([19
  Section 3](../19-performance/03-concurrency-performance.md)'s numbers);
...
promotion; [08 Section 4](../08-concurrency/04-sync-primitives.md)'s
comparison table).
...
without solving seeding yourself ([21 Section 1](../21-security/01-threat-model-and-validation.md)'s
boundary rules).
...
- `-race` detects the concurrent-map-write panic before it reaches
  production ([10 Section 1](../10-testing/01-fundamentals.md)).
